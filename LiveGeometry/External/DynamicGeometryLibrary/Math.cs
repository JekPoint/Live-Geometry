...
namespace DynamicGeometry
{
    ...
    public partial struct PointPair
    {
        ...
        public PointPair Reverse => new PointPair(P2, P1);
    ...
    public static partial class Math
    {
        ...
        public static Point InfinitePoint => new Point(double.NaN, double.NaN);

        public static PointPair InfinitePointPair => new PointPair() { P1 = Math.InfinitePoint, P2 = Math.InfinitePoint };
        ...
        public static bool IsPointInPolygonOld(this IList<Point> polygon, Point start)
        {
            ...
            for (var i = 0; i < n; i++)
            {
                var side = new PointPair(polygon[i], polygon[i.RotateNext(n)]);

                var intersectionPoint = GetIntersectionOfLines(ray, side);
                ...
                var isPointOnSide = IsPointInSegmentBoundingRect(side, intersectionPoint);
                var isPointOnRay = IsPointInSegmentBoundingRect(ray, intersectionPoint);
        ...
        public static List<Point> GetIntersectionsOfPolygonAndSegment(IList<Point> polygon, PointPair segment, bool inclusive)
        {
            ...
            {
                var side = new PointPair(polygon[i], polygon[i.RotateNext(n)]);
                var intersection = InfinitePoint;
        ...
        public static List<Point> GetIntersectionsOfPolygonAndLine(IList<Point> polygon, PointPair line, bool inclusive)
        {
            ...
            {
                var side = new PointPair(polygon[i], polygon[i.RotateNext(n)]);
                var intersection = GetIntersectionOfSegmentAndLine(side, line, inclusive);
                if (intersection.Exists())
                {
                    if (inclusive)
                    {
        ...
        {
            var result = new List<Point>();
            var n = polygon1.Count;
            for (var i = 0; i < n; i++)
            {
                var side = new PointPair(polygon1[i], polygon1[i.RotateNext(n)]);
                var intersections = GetIntersectionsOfPolygonAndSegment(polygon2, side, inclusive);
        ...
        public static List<Point> GetIntersectionsOfPolygonAndEllipse(IList<Point> polygon, IEllipse ellipse, bool inclusive)
        {
            ...
            {
                var side = new PointPair(polygon[i], polygon[i.RotateNext(n)]);
                var intersections = GetIntersectionOfEllipseAndSegment(ellipse, side);
        ...
        {
            get => Settings.Instance.CursorTolerance;
            set => Settings.Instance.CursorTolerance = value;
        }
        ...
        {
            var angle = Math.GetAngle(center, point);
        ...
        {
            var result = new Point(center.X, center.Y);
        ...
        public static double PI => M.PI;

        public static double DOUBLEPI => 6.28318530718;
        ...
        {
            var nearestProjection = new ProjectionInfo();
        ...
        {
            var result = GetIntersectionOfLines(segment1, segment2);
            if (!result.Exists())
            {
                return result;
            }
        ...
        {
            var result = GetIntersectionOfLines(segment, line);
            if (!result.Exists())
            {
                return result;
            }
        ...
        {
            var result = GetIntersectionOfLineAndRect(segment, rect);
        ...
        public static PointPair GetIntersectionOfEllipseAndLine(IEllipse ellipse, PointPair line)
        {
            ...
            var semiMajor = ellipse.SemiMajor;
            var semiMinor = ellipse.SemiMinor;
            var angle = ellipse.Inclination;
        ...
        public static PointPair GetIntersectionOfEllipseAndLine(
            ...
            Point point)
        {
            var point2 = new Point(point.X + 1, point.Y + slope);
        ...
        public static Point GetPointOnPolylineFromParameter(IList<Point> logicalPoints, double parameter)
        {
            ...
            for (var i = 0; i < logicalPoints.Count - 1; i++)
            {
        ...
        public static Point GetPositionByExactAngleAndLength(Point center, Point point, double Angle, double Length)
        {
            ...
            var dx = Length * M.Cos(Math.ToRadians(Angle));
            var dy = Length * M.Sin(Math.ToRadians(Angle));
        ...
        public static Point GetSnapToSegmentCenterPosition(Point point, List<Segment> segments)
        {
            foreach (var sg in segments)
            {
                ...
                {
                    var tolerance = 0.1;
                    var P1x = sg.Coordinates.P1.X;
                    var P1y = sg.Coordinates.P1.Y;
                    var P2x = sg.Coordinates.P2.X;
                    var P2y = sg.Coordinates.P2.Y;
                    var Px = point.X;
                    var Py = point.Y;

                    var deltaX1 = M.Abs(P2x - Px);
                    var deltaY1 = M.Abs(P2y - Py);

                    var deltaX2 = M.Abs(P1x - Px);
                    var deltaY2 = M.Abs(P1y - Py);
