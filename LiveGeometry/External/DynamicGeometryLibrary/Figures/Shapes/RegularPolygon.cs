...
namespace DynamicGeometry.Figures.Shapes
{
    public class RegularPolygon : CompositeFigure, IShapeWithInterior
    {
        ...
        public override Point Center => this.Dependencies.Point(0);

        public Point Vertex => this.Dependencies.Point(1);
        ...
        public override void Recalculate()
        {
            ...
            var initialAngle = Math.GetAngle(center, vertex);
            var radius = center.Distance(vertex);
            var increment = Math.DOUBLEPI / NumberOfSides;

            for (var i = 0; i < NumberOfSides - 1; i++)
            {
                var angle = initialAngle + (i + 1) * increment;

                var X = center.X + radius * System.Math.Cos(angle);
                var Y = center.Y + radius * System.Math.Sin(angle);
        ...
        {
            var allVertices = new List<IFigure>();
        ...
        private void AdjustSides()
        {
            if (sides.Count < NumberOfSides)
            {
                for (var i = 0; i < NumberOfSides - sides.Count; i++)
                {
                    AddSide();
                }
            }
            ...
            {
                for (var i = 0; i < sides.Count - NumberOfSides; i++)
                {
                    RemoveSide();
                }
            }
        ...
        private void AdjustVerticesList()
        {
            ...
            {
                var requiredNumber = NumberOfSides - vertices.Count - 1;
            ...
            {
                var requiredNumber = vertices.Count - NumberOfSides;
                for (var i = 0; i <= requiredNumber; i++)
                {
                    RemoveVertex();
                }
