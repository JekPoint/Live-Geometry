...
namespace DynamicGeometry.Figures.Points
{
    public class DilatedPoint : PointBase, IPoint
    {
        ...
        public IPoint Source => (Dependencies.Count >= 1) ? Dependencies.ElementAt(0) as IPoint : null;

        //[PropertyGridVisible]
        [PropertyGridName("About Point ")]
        public new IPoint Center => (Dependencies.Count >= 2) ? Dependencies.ElementAt(1) as IPoint : null;

        //[PropertyGridVisible]
        public double Factor_Numerator => (Dependencies.Count >= 3) ? (Dependencies.ElementAt(2) as ILengthProvider).Length : 1;

        //[PropertyGridVisible]
        public double Factor_Denominator => (Dependencies.Count >= 4) ? (Dependencies.ElementAt(3) as ILengthProvider).Length : 1;
