...
namespace DynamicGeometry.Figures.Points
{
    public partial class PointBase : CoordinatesShapeBase<Shape>, IPoint
    {
        ...
        {
            get => Coordinates.X;
            set => Coordinates = Coordinates.SetX(value);
        }

        public virtual double Y
        {
            get => Coordinates.Y;
            set => Coordinates = Coordinates.SetY(value);
        }

        double PointSize => Shape.ActualWidth / 2;

        public override IFigure HitTest(Point point)
        {
            var tolerance = CursorTolerance + ToLogical(PointSize);
