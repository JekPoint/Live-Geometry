...
namespace DynamicGeometry.Figures.Circles
{
    ...
    public abstract partial class EllipseArcBase : ShapeBase<Path>, IArc, ILengthProvider
    {
        ...
        public virtual double SemiMajor => Math.Distance(Point(0), Point(1));

        public virtual double SemiMinor => Math.Distance(Point(0), Point(2));
        ...
        public virtual double Length => double.NaN;
        ...
        public override IFigure HitTest(Point point)
        {
            ...
            var shapeStyle = Style as ShapeStyle;
            if (shapeStyle != null)
            {
            ...
            var between = Math.IsAngleBetweenAngles(angleToPoint, StartAngle, EndAngle, Clockwise);
            if (between)
            {
        ...
        public virtual double Inclination => Math.GetAngle(Center, Point(1));
        ...
        public virtual double StartAngle => Math.GetAngle(Center, BeginLocation);

        public virtual double EndAngle => Math.GetAngle(Center, EndLocation);

        /// <summary>
        /// The central angle of the arc.
        /// </summary>
        public virtual double Angle => Clockwise ? Math.OAngle(EndLocation, Center, BeginLocation) :
    ...
    public abstract partial class CircleArcBase : EllipseArcBase, ICircle
    {
        ...
        public override Point BeginLocation => Point(BeginPointIndex);

        public override int BeginPointIndex => 1;

        public override Point EndLocation => Math.ScalePointBetweenTwo(
            Center,
            Point(2),
            Radius / Center.Distance(Point(2)));

        public override int EndPointIndex => 2;

        public override double Length => Radius * Angle;

        public virtual double Radius => SemiMajor;

        public override double SemiMinor => SemiMajor;
    }
