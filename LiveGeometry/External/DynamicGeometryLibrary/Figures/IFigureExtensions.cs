...
namespace DynamicGeometry.Figures
{
    public static class IFigureExtensions
    {
        ...
        public static IEnumerable<Point> EnumeratePointsOnLinearFigure(this ILinearFigure figure)
        {
            ...
            for (var lambda = domain.Item1; lambda < domain.Item2; lambda += 0.01)
            {
        ...
        {
            var temp = new List<IFigure>(figure.Dependencies);
            if (index < 0 || index >= temp.Count)
            {
                throw new ArgumentOutOfRangeException("index");
            }
            var oldDependency = temp[index];
            ...
            {
                foreach (var child in compositeFigure.Children)
                {
                    temp.Clear();
        ...
        {
            var index = figure.Dependencies.IndexOf(oldDependency);
            if (index == -1)
            {
        ...
        {
            var dependents = new List<IFigure>(figure.Dependents.Where(f => !(f is PointLabel)));
        ...
        public static string GenerateNewName(this IFigure figure)
        {
            ...
            var className = figure.GetType().Name;
#if TABULA
        ...
        {
            foreach (var f in figure.Dependencies)
            {
                if (f is PointBase)
                {
