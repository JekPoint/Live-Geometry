...
namespace DynamicGeometry.Styles
{
    public partial class StyleManager
    {
            ...
            where TStyle : class, IFigureStyle
        {
            foreach (var style in list)
            {
                var correctStyle = style as TStyle;
        ...
        public IFigureStyle CreateNewStyle(IFigure figure)
        {
            ...
            Actions.Actions.AddItem(figure.Drawing.ActionManager, list, newStyle);
            return newStyle;
        }
        ...
        public IFigureStyle CreateNewStyle(Drawing drawing, IFigureStyle style)
        {
            ...
            Actions.Actions.AddItem(drawing.ActionManager, list, newStyle);
            return newStyle;
        }
        ...
        public IFigureStyle FindExistingOrAddNew(IFigureStyle style)
        {
            ...
            Actions.Actions.AddItem(Drawing.ActionManager, list, style);
            return style;
        }
        ...
        public virtual void Remove(IFigureStyle style)
        {
            if (list.Contains(style) && list.IndexOf(style) >= numDefaultStyles)
            {
                ...
                foreach (var fig in Drawing.Figures)
                {
                    if (fig.Style == style)
                    {
                        Actions.Actions.SetProperty(Drawing.ActionManager, new PropertyValue("Style",fig), AssignDefaultStyle(fig));
                    }
                }
                Actions.Actions.RemoveItem(Drawing.ActionManager, list, style);
                transaction.Commit();
            }
        }
